    ...
    const fetchAudits = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/audit?page=0&size=20&sort=createdAt,desc",
          {
            headers: {
    ...
    const fetchRecentActivities = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:8080/api/audit?page=0&size=4&sort=createdAt,desc",
          {
            method: "GET",
                ...
                <table className="table align-middle mb-0">
                  ...
                  <tbody>
                    {audits?.content
                      ?.filter(
                        (audit) =>
                          audit.targetType === "SCHOOL" && audit.targetName,
                      )
                      .slice(0, 4)
                      .map((audit, idx) => (
                        <tr key={audit.id || idx}>
